import os, time, hashlib, re, shutil, math, random
...
MAX_WORKERS = 8                # NEW: parallel downloads
PAGE_SIZE = 100                # SerpAPI up to ~100 per page for images
K_COLORS = 5
RANDOM_SEED = 42               # NEW: reproducible KMeans
MAX_SIDE = 768                 # NEW: speed-up (resize)
SAMPLE_PIXELS = 120_000        # NEW: subsample pixels for kmeans
# ========================================
...
def serpapi_search_urls(query: str, target_count: int) -> list[str]:
    ...
    for i in range(pages):
        ...
        for attempt in range(3):  # NEW: retries
            try:
                r = session.get("https://serpapi.com/search.json", params=params)
                r.raise_for_status()
                data = r.json()
...
def _prepare_for_kmeans(img: Image.Image) -> np.ndarray:
    ...
    # Subsample pixels for speed but keep distribution
...
def dominant_colors(path: Path, k: int = K_COLORS):
    ...
    # NEW: seeded init for reproducibility
    ...
    # Convert LAB centers back to HEX
...
def hex_to_hsv(hexcode: str):
    ...
    hsv = cv2.cvtColor(np.uint8([[[int(r*255),int(g*255),int(b*255)]]]), cv2.COLOR_RGB2HSV)[0,0]
    h, s, v = float(hsv[0]), float(hsv[1])/255.0, float(hsv[2])/255.0
    return h, s, v

def color_family(hexcode: str) -> str:
    h, s, v = hex_to_hsv(hexcode)
    if v < 0.18 or (s < 0.08 and v < 0.9):         # very dark or low-sat neutrals
        return "neutrals"
    if s < 0.18 and v >= 0.9:
        return "pastels"
    if s > 0.7 and v > 0.7:
...
for name in ["download_manifest.csv", "colors_opencv.csv", "colors_families.csv",
             "top_colors_today.csv", "top_color_families_today.csv"]:
    src = OUT_DIR / name
    if src.exists():
