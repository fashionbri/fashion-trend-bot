...
ACC = ["earrings","necklace","bracelet","ring","belt","hat","bag","clutch","sunglasses","gloves","scarf","boots","heels","sneakers"]
...
def _setup():
    ...
    text = tokenizer([f"a runway close-up showing {a}" for a in ACC])
...
def run(paths, out_dir):
    ...
    for p in paths:
        try:
            ...
            rows.append({"image_path":p,"primary_label":ACC[top],"score":float(prob[top])})
        except Exception: continue
    df=pd.DataFrame(rows); df.to_csv(Path(out_dir)/"accessories.csv", index=False)
    if not df.empty:
        agg=df.groupby("primary_label")["image_path"].count().reset_index(name="count")
        agg["percent"]=agg["count"]/agg["count"].sum()
        agg.to_csv(Path(out_dir)/"accessories_summary.csv", index=False)
