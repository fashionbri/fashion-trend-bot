...
CLASSES = ["dress","coat","blazer","suit","trench coat","knit top","blouse","skirt","pants","jeans","shorts","gown","jumpsuit","outerwear","handbag","shoes"]
...
def _setup():
    ...
    text = tokenizer([f"a runway look featuring a {c}" for c in CLASSES])
...
def run(paths, out_dir):
    ...
    for p in paths:
        try:
            ...
            rows.append({"image_path":p,"primary_label":CLASSES[top],"score":float(prob[top])})
        except Exception: continue
    df=pd.DataFrame(rows); df.to_csv(Path(out_dir)/"garments.csv", index=False)
    if not df.empty:
        agg=df.groupby("primary_label")["image_path"].count().reset_index(name="count")
        agg["percent"]=agg["count"]/agg["count"].sum()
        agg.to_csv(Path(out_dir)/"garments_summary.csv", index=False)
