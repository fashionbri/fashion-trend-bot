...
MOODS = ["romantic","rebellious","dreamy","elegant","futuristic","sporty","nostalgic","sensual","minimalist","maximalist"]
...
def _setup():
    ...
    text = tokenizer([f"a fashion editorial that feels {m}" for m in MOODS])
...
def run(paths, out_dir):
    ...
    for p in paths:
        try:
            ...
            rows.append({"image_path":p,"primary_label":MOODS[top],"score":float(prob[top])})
        except Exception: continue
    df=pd.DataFrame(rows); df.to_csv(Path(out_dir)/"mood.csv", index=False)
    if not df.empty:
        agg=df.groupby("primary_label")["image_path"].count().reset_index(name="count")
        agg["percent"]=agg["count"]/agg["count"].sum()
        agg.to_csv(Path(out_dir)/"mood_summary.csv", index=False)
